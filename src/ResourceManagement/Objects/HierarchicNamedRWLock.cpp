...
{
  auto pNode = internal::HierarchicNamedRWLockNode::GetOrCreateNode(rootNode, resourceName);
  ...
  if (rootNode.GetNbOfLocksInChilds() == std::numeric_limits<uint32_t>::max())
...
{
  auto pNode = internal::HierarchicNamedRWLockNode::GetExistingNode(rootNode, resourceName);
 ...
 *
 * \return
 * true  = There are no read- and no write-locks.\n
 * false = There is at least one read- or write-lock.
 */
bool HierarchicNamedRWLock::IsAnyLock(void) const noexcept
{
  return rootNode.IsAnyLockInChilds();
}
