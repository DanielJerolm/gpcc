...
, spFreeBlocks(std::make_unique<internal::FreeBlockPool>(maxSizeInFirstBucket, nBuckets))
, spDescriptorPool(std::make_unique<internal::MemoryDescriptorPool>())
, statistics(1, size)
/**
 * \brief Constructor.
 *
 ...
 */
{
  ...
  spFreeBlocks->Add(spDescriptorPool->Get(baseAddress, size, true));
}
...
{
  // Intentionally empty.
  // This DTOR can't de declared with "=default" in the hpp-file, because the DTORs of std::unique_ptrs can't live
  // with a forward declaration of the internals.
}
 ...
 */
{
  ...
  MemoryDescriptor* const pBlock = spFreeBlocks->Get(size);
  if (pBlock == nullptr)
    return nullptr;
  ...
  if (pBlock->size > size)
  {
    ...
    ON_SCOPE_EXIT() { spFreeBlocks->Add(pBlock); };
    MemoryDescriptor* const pNewBlock = spDescriptorPool->Get(pBlock->startAddress + size, pBlock->size - size, true);
    ON_SCOPE_EXIT_DISMISS();

    pBlock->InsertIntoMemListBehindThis(pNewBlock);
    pBlock->size = size;

    spFreeBlocks->Add(pNewBlock);
  }
  else
  {
 ...
 */
{
  ...
  if ((pDescr->pPrevInMem != nullptr) && (pDescr->pPrevInMem->free))
  {
    ...
    spDescriptorPool->Recycle(pPrev);
  ...
  if ((pDescr->pNextInMem != nullptr) && (pDescr->pNextInMem->free))
  {
    ...
    spFreeBlocks->Remove(pNext);

    pDescr->size += pNext->size;

    pNext->RemoveFromMemList();

    spDescriptorPool->Recycle(pNext);
