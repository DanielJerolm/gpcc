...
#error "GPCC requires that Zephyr's option 'TIMEOUT_64BIT' is set."
#endif
...
{
  int const status = k_condvar_init(&condVar_);

  if (status != 0)
    throw std::system_error(status, std::generic_category(), "k_condvar_init() failed");
}
...
bool ConditionVariable::TimeLimitedWait(Mutex & mutex, time::TimePoint const & absoluteTimeout)
{
  ...
  // get current time
  TimePoint const curr_time = TimePoint::FromSystemClock(clockID);
...
{
  int const status = k_condvar_signal(&condVar_);

  if (status != 0)
    throw std::system_error(status, std::generic_category(), "k_condvar_signal() failed");
}
...
{
  int const status = k_condvar_broadcast(&condVar_);

  if (status < 0)
    throw std::system_error(status, std::generic_category(), "k_condvar_broadcast() failed");
}
