 ...
 */
ObjectVAR_wicb_withASM::ObjectVAR_wicb_withASM(std::string                const & _name,
...
: ObjectVAR_wicb(_name, _type, _nElements, _attributes, _pData, _pMutex,
                 _onBeforeReadCallback, _onBeforeWriteCallback, _onAfterWriteCallback)
...
size_t ObjectVAR_wicb_withASM::GetAppSpecificMetaDataSize(uint8_t const subIdx) const
{
  if (subIdx != 0U)
...
std::vector<uint8_t> ObjectVAR_wicb_withASM::GetAppSpecificMetaData(uint8_t const subIdx) const
{
  if (subIdx != 0U)
    throw gpcc::cood::SubindexNotExistingError();

  if (appSpecMetaData.size() == 0U)
    throw std::logic_error("ObjectVAR_wicb_withASM::GetAppSpecificMetaData: No ASM");
