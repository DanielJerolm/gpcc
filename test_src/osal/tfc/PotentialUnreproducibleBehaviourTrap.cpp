 ...
 * more than one @ref PotentialUnreproducibleBehaviourTrap instance in the process which has monitoring enabled.
 *
 * - - -
 *
 ...
 */
bool PotentialUnreproducibleBehaviourTrap::QueryAndReset(void)
{
  if (!enabled)
    throw std::logic_error("Trap is not enabled");

  auto pTFCCore = gpcc::osal::internal::TFCCore::Get();
  return pTFCCore->QueryAndResetWatchForBlockWithSameTimeout();
}
...
void PotentialUnreproducibleBehaviourTrap::EndMonitoring(void)
{
  ...
  if (trapped)
  {
    ADD_FAILURE() << "TFC: Trap for potential unreproducible behaviour has been triggered." << std::endl
